...
namespace DistantLands.Cozy
{
    ...
    public class BlocksAttributeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Rect blockRef = new Rect(position.x, position.y, position.width / 2, position.height);
            Rect weightRef = new Rect(position.x + 10 + position.width / 2, position.y, -20 + position.width / 2, position.height);

            EditorGUI.PropertyField(blockRef, property.FindPropertyRelative("colorBlock"), GUIContent.none);
            EditorGUI.PropertyField(weightRef, property.FindPropertyRelative("weight"), GUIContent.none);
